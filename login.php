...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($captcha_token)) {
        ...
        // 3. 使用curl发送验证请求（禁用SSL验证，适合测试）
        $ch = curl_init($verify_url);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($data));
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_TIMEOUT, 10);
        // 关键修复：禁用SSL证书验证（临时测试用）
        ...
        if (!empty($curl_error)) {
            ...
            if (!$result || !$result->success) {
                ...
                if (empty($username) || empty($password)) {
                    ...
                    $stmt = $conn->prepare("SELECT id, username, password, points, is_admin FROM users WHERE username = ?");
...
<body>
    ...
    <div class="login-card">
        ...
        <?php if ($msg): ?>
            <div class="message success-msg"><?php echo htmlspecialchars($msg); ?></div>
        <?php endif; ?>
        <?php if ($error): ?>
        ...
        <form method="post" id="loginForm">
            <div class="input-group">
                ...
                <input type="text" placeholder="电子邮箱或用户名" name="username" id="username" value="<?php echo isset($_POST['username']) ? htmlspecialchars($_POST['username']) : ''; ?>">
            </div>
            <div class="input-group">
            ...
            <div class="btn-container">
                <button type="submit" class="signin-btn" id="signinBtn" disabled>
                    登录 
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="lucide lucide-arrow-right" viewBox="0 0 24 24">
